...
\end{definition}
\begin{definition}[\colordefinition{adjoint operator}]
    ...
    \end{align*}
\end{definition}
\begin{lemma}[\important{Cauchy-Schwartz inequality}] If \((\cdot, \cdot)\) is a pre-inner product, then \(\vert (u,v)\vert \leq (u,u)^{1/2}(v,v)^{1/2}\).
\end{lemma}
\begin{corollary}
    Assume we have a seminorm \(||u||\eqdef (u,u)^{1/2}\). It is a norm iff \((\cdot,\cdot)\) is an inner product.
\end{corollary}
\begin{definition}[\colordefinition{Hilbert space}]
...
\begin{theorem}[\ifcolor\color{red}\textbf{projection theorem}\color{black}\else projection theorem\fi]
    ...
    \end{align*}
    This minimizer \(u_0 = P_{\mathcal{C}}u\) is called the \colordefinition{orthogonal projection }of \(u\) onto \(\mathcal{C}\).
\end{theorem}
\ifdetailed 
\begin{proof}
...
For a Hilbert space \(\mathcal{H}\) and a subset \(A\subset H\), the following is the \colordefinition{orthogonal complement } of \(A\):
\begin{align*}
...
\begin{proposition}[\important{decomposition of Hilbert spaces}]
...
\(||Pu||\leq ||u||\), since \(||u||^2 = ||Pu||^2 + ||u-Pu||^2\). It is called the \colordefinition{orthogonal projection }onto \(\mathcal{H}_0\).

If \(\mathcal{H}_0\) is \important{finite dimensional with an orthonormal basis } \(u_1, ..., u_n\) then 
\begin{align*}
...
\end{definition}
\begin{theorem}
    \important{Every Hilbert space \(\mathcal{H}\) has an orthonormal basis}. If \(\mathcal{H}\) is separable, then there is a countable orthonormal basis.
\end{theorem}
\begin{proposition}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item \important{\emph{Parseval's identity}}: If \(\{u_i\}_{i\in I }\) is an orthonormal basis, then \\ \(\sum_{i\in I} |(u,u_i)|^2 = ||u||^2\).
        \item \(\bigcup\limits_{N=1}^{\infty}E(N)\) is \important{dense } in \(\mathcal{H}\) where \(E(N) = \text{span}\{e_1, ..., e_N\}\)
        \item \(g = \sum\limits_{n=1}^{\infty}\langle g, e_n\rangle e_n \ \forall g\in \mathcal{H}\). (\important{Fourier coefficients})
    \end{enumerate}
\end{proposition}
...
\end{corollary}
\begin{corollary}
    Up to a unitary isomorphism, there is \important{only one infinite dimensional separable Hilbert space}, namely, \(l^2\). Recall a unitary isomorphism is a bijective map between spaces, \(U:H_1\rightarrow H_2\) s.t. \(\langle Ux, Uy\rangle_{H_2} = \langle x,y\rangle_{H_1}\).
\end{corollary}
...
\emph{Fact}: Any two orthonormal bases in a Hilbert space have the same cardinality.
...
If (i)-(iii) are satisfied, then \(f\) is called a \colordefinition{\emph{bounded linear functional}}. The constant \(c\) in (iii) is denoted by \(||f||\).
...
For every Hilbert space \(\mathcal{H}\) we can define the \colordefinition{\emph{conjugate Hilbert space }} \(\overline{\mathcal{H}}\), which has its elements as 
