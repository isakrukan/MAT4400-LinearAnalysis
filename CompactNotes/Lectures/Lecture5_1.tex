\section{Measurable Mappings \tiny{ (7, \cite{schilling2017measures})}}
    ...
    \begin{definition}
        The measure $\mu'(\cdot)$ in the above theorem is called the \colordefinition{pushforward or image measure} of $\mu$ under $T$ and it is denoted as 
    ...
    \begin{corollary}
        Lebesgue measure is invariant under motions: $\lambda^n=M(\lambda^n)$ for all motions $M$ in $\mathbb R^n$. In particular, congruent sets have the same measure. Two sets of points are called \colordefinition{congruent } if, and only if, one can be transformed into the other by an isometry.
    \end{corollary}