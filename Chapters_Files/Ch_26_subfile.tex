...
\begin{theorem}[projection theorem]
    ...
    \begin{align*}
        ||u-u_0|| = d(u,\mathcal{C})(=\inf\limits_{v\in\mathcal{C}} ||u-x||).
    \end{align*}
\end{theorem}
\ifdetailed 
\begin{proof}
...
\begin{definition}[orthonormal system]
    An orthonormal system in \(\mathcal{H}\) is a collection of vectors \(h_i\in H \) (\(i\in I\))s.t.
    \begin{align*}
...
\end{definition}
\begin{definition}
    A Hilbert space \(\mathcal{H}\) is said to be \emph{separable} if \(\mathcal{H}\) contains a countable dense subset \(G\subset\mathcal{H}\).
\end{definition}
\begin{theorem}
...
\end{theorem}
\begin{proposition}
    Assume \(\{u_i\}_{i\in I}\) is an orthonormal system in a Hilbert space H. Take \(u\mathcal{H}\). Then 
    \begin{enumerate}[label=(\roman*)]
...
\begin{theorem}[Riesz]
    ...
    \begin{align*}
        f(x) = (x,y),
    \end{align*}
    for a uniquely defined \(u\in\mathcal{H}\). Moreover, we have \(||f||=||y||\).
\end{theorem}
